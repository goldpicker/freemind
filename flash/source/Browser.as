 ...
 */
/*$Id: Browser.as,v 1.1 2005-04-25 20:32:21 christianfoltin Exp $*/
/**
	Browser Class
...
class Browser {
	...
	private  static var mcl:MovieClipLoader=new MovieClipLoader();

	public var mc_container:MovieClip;
	public var mc_floor:MovieClip; // dragable.
	public var floor:Floor;
	private var first_left_node:Node=null;
	private var first_right_node:Node=null;
	...
	private var numWaitingImages:Number=0;
	private var resizer;
	private var bBack;
	...
	private var bShadow;
	private var bInfo;
	public static var withShadow=false;
	...
	function createSizeButtons(){
		...
		bShadow.onPress=function(){
			...
			this.moveTo(0,0);
			this.lineTo(1,0);
			Browser.withShadow= Browser.withShadow?false:true;
			if(Browser.withShadow==true){
	...
	function createInfoWindow(){
		...
		if(mc_container.info==null){
			...
			mc_container.info.tex_container.createTextField("txt", 1, 0, 0,100,10);
			...
			txt.wordWrap = true;
			txt.html = true;
			txt.htmlText="<font color='#BB8833'><b>SHORTCUTS</b><br>"+
			"LEFT ->move left<br>"+
			"RIGHT ->move all right<br>"+
			"UP<br>"+
			"DOWN<br>"+
			"CTRL LEFT<br>"+
			"CTRL RIGHT<br>"+
	...
	function saveOldPosition(){
		...
		ant_pnode_y=first_right_node.ref_mc._y;
		ant_floor_x=mc_floor._x;
		ant_pnode_x=first_right_node.ref_mc._x;
	}
	...
	function relocateFloor(){
		if(initialization){
			mc_floor._x=0;
			mc_floor._y=0;
			mc_floor._x=Stage.width/2-first_right_node.ref_mc._x;
			mc_floor._y=Stage.height/2-first_right_node.ref_mc._y;
			initialization=false;
		}else{
			mc_floor._y=ant_floor_y+ant_pnode_y-first_right_node.ref_mc._y;
			mc_floor._x=ant_floor_x+ant_pnode_x-first_right_node.ref_mc._x;
		}
	}
	...
	function evalXML(){
		...
		//Right
		genNodes(true,nodeXMLIni,0,0,color_LineaIni,lineWidth,styleNode,styleLine,true,mc_floor);
		//Left
	...
	function drawArrows(){
		for(var i=0;i<list_arrows.length;i++){
			...
			if(mc_orig._visible && mc_dest._visible)
				drawArrow(mc_orig,mc_dest,arrow[4]);
		}
	}

	function drawArrow(mc_orig,mc_dest,color){
		var xcenter=first_right_node.ref_mc._x;

		mc_floor.lineStyle(1,color,100);

		mc_floor.moveTo(mc_orig._x+(mc_orig._x>xcenter?mc_orig._width:0),
						mc_orig._y+mc_orig._height/2);
		mc_floor.lineTo(mc_dest._x+(mc_dest._x>xcenter?mc_dest._width:0),
	...
	function initFirstNode(isRight,node){
		if(isRight)
			first_right_node=node;
		else
			first_left_node=node;
	}
